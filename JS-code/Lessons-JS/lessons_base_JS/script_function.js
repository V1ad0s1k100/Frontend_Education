...
function logMessageInfo(
  message,
  count /* Внутри скобок параметры объявляются как let переменные, поэтому при использовании их в функции не обязательно указывать тип переменной */,
) {
  messageLog = `(((${message})))`
  for (let i = 0; i < count; i++) {
...
function logMessageInfo(
  message,
  count = 4 /* Задаем значение по умолчанию. В случае если не был передан необходимый аргумент, то параметр примет значение по умолчанию */,
) {
